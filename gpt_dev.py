...
# y is shift 1 from block size window on x
...
# note the time dimension for nlp case is 1 char to the right hence we iterate block size
...
# reason why we normalise with standarddiv cause when values too high start to grab max converge instead of diffuse
...
class LayerNorm1d: # (used to be BatchNorm1d)
  ...
  def __call__(self, x):
    ...
    # changing to 1 we normalise by row instead of column which is 0
    xmean = x.mean(1, keepdim=True) # batch mean
    xvar = x.var(1, keepdim=True) # batch variance
    xhat = (x - xmean) / torch.sqrt(xvar + self.eps) # normalize to unit variance + eps so that wont div by zero
